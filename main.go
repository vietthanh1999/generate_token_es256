...
func main() {
	...
	r.POST("/generate-token", func(c *gin.Context) {
		...
		for i := 0; i < len(body.AppUserIdList); i++ {
			token, _ := generateJWT(body.AppName, body.AppUserIdList[i], body.ContentIdList[rand.Intn(len(body.ContentIdList))], os.Getenv("GENERATOR_KEY"))
	...
	r.POST("/generate-token-to-interactive", func(c *gin.Context) {
		...
		for i := 0; i < len(body.Profiles); i++ {
			token, _ := generateJWTToInteractive(body.Profiles[i], os.Getenv("GENERATOR_INTERACTIVE_KEY"))
			tokenList = append(tokenList, token)
		}
